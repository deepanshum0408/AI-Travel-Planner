...
2. Add your API keys and environment variables to the `.env` file:
    ```plaintext
    # Required: Groq API Key (for LLM functionality)
    GROQ_API_KEY=your_groq_api_key_here
    
    # Required: OpenAI API Key (for email generation)
    OPENAI_API_KEY=your_openai_api_key
    
    # Required: SERPAPI API Key (for flights and hotels search)
    SERPAPI_API_KEY=your_serpapi_api_key
    
    # Required: SendGrid API Key (for sending emails)
